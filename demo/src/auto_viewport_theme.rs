...
#[cfg(not(target_arch = "wasm32"))]
mod native {
    use egui::{Context, Id, SystemTheme, ThemePreference, Ui, ViewportCommand};
    use std::sync::Arc;
    ...
    impl State {
        fn end_frame(ui: &mut Ui) {
            let preference = ui.options(|opt| opt.theme_preference);
            let has_changed = !ui
                .data(|d| d.get_temp::<State>(Id::NULL))
                .map(|s| s.preference)
                .is_some_and(|old| old == preference);
            if has_changed {
                ui.send_viewport_cmd(ViewportCommand::SetTheme(to_system_theme(preference)));
                ui.data_mut(|d| d.insert_temp(Id::NULL, State { preference }));
            }
        }
    }
